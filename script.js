...
const shareMeetingLink = document.getElementById('share-meeting-link');
const copyShareLinkBtn = document.getElementById('copy-share-link-btn');
const dismissShareLinkBtn = document.getElementById('dismiss-share-link-btn');
...
function setupInteractiveBackground() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        bg.style.background = `radial-gradient(circle at ${x}% ${y}%, rgba(138, 180, 248, 0.15) 0%, transparent 60%)`;
    });
}

function startHackingTextEffect() {
    const textElement = document.getElementById('hacking-text');
    if (!textElement) return;

    const fullText = "MADE WITH LOVE ❤️ BY RG";
    ...
    cursor.className = 'hacking-cursor';
                ...
                if (charIndex < targetWord.length) {
                    ...
                    const shuffleInterval = setInterval(() => {
                        if (iterations < maxIterations) {
                            wordSpan.textContent = targetWord.substring(0, charIndex) + chars[Math.floor(Math.random() * chars.length)];
                            iterations++;
                        } else {
                            clearInterval(shuffleInterval);
                            wordSpan.textContent = targetWord.substring(0, charIndex + 1);
                            charIndex++;
                            typeNextChar();
                        }
                        textElement.appendChild(cursor);
...
function showLobby() {
    ...
    const acquireStream = async (constraints) => {
        try {
            ...
            console.warn("Permission failure with constraints:", constraints, err);
            if (constraints.video && constraints.audio) {
                // Try video only
                lobbyStatus.innerText = "Mic failed, trying Camera only...";
                return acquireStream({ video: true, audio: false });
            } else if (constraints.video) {
                // Try audio only
                lobbyStatus.innerText = "Camera failed, trying Mic only...";
                return acquireStream({ video: false, audio: true });
            } else {
                // Both failed or was already single request
                console.warn("All primary media requests failed, using dummy stream");
...
async function startMeeting(joinId) {
    ...
    // FIX: Only disable tracks that are actually dummy tracks
    if (usingDummyVideo) {
        const vTrack = localStream.getVideoTracks()[0];
        if (vTrack) vTrack.enabled = false;
        videoToggleBtn.classList.add('active');
        updateVideoState('local', false);
    } else {
        updateVideoState('local', true);
    }
    ...
    if (usingDummyAudio) {
        ...
        updateAudioState('local', true);
    }

    const localWrapper = document.getElementById('wrapper-local');
    if (localWrapper && usingDummyAudio) {
    ...
    peer.on('open', (id) => {
        ...
        if (joinId) {
            ...
            const shortId = joinId.includes('-') ? joinId.split('-')[0] : joinId.slice(0, 8);
            meetingIdDisplay.innerText = `Connected: ${shortId}...`;
        } else {
            // Host: Ready immediately, hide loading
            const shortId = id.includes('-') ? id.split('-')[0] : id.slice(0, 8);
            meetingIdDisplay.innerText = `Code: ${shortId}`;
            ...
            // Show share link modal for host
            showShareLinkModal(id);
        }
    });
    ...
    call.on('stream', (remoteStream) => {
        if (isScreenCall) {
            ...
            if (participantsData[call.peer]) {
                ...
                updateVideoState(call.peer, data.videoEnabled !== false);
...
function connectToPeer(peerId) {
    ...
    call.on('stream', (remoteStream) => {
        ...
        if (participantsData[peerId]) {
            ...
            updateVideoState(peerId, data.videoEnabled !== false);
    ...
    conn.on('data', (data) => {
        if (data.type === 'peer-list') {
            ...
            participantsData[conn.peer] = {
                ...
                videoEnabled: data.videoEnabled !== false // Store video state
            };

            // FIX: Ensure UI reflects the received video/audio state immediately
            updateVideoState(conn.peer, data.videoEnabled !== false);
            updateAudioState(conn.peer, data.audioEnabled !== false);
            ...
            if (pendingStreams[conn.peer] && pendingStreams[conn.peer].main) {
                ...
                // Immediately apply the video state after adding stream
                updateVideoState(conn.peer, data.videoEnabled !== false);
            ...
            // Auto-hide share link modal if this is the first joiner
            onParticipantJoined();
        }
...
function updatePeerUI(peerId, name, avatar, retries = 15) {
    const wrapperId = peerId === 'local' ? 'wrapper-local' : `wrapper-${peerId}`; const wrapper = document.getElementById(wrapperId);
    if (wrapper) {
...
function addVideoStream(stream, isMuted, name, avatar, isLocal, peerId, isScreenBox) {
    ...
    // For local, create a specific stream copy to avoid potential conflicts with PeerConnections
    if (isLocal) {
    ...
    // Ensure local video element has extra properties to avoid black screen
    if (isLocal) {
        console.log("Adding local video stream. Tracks:", stream.getTracks());
        const vTrack = stream.getVideoTracks()[0];
        console.log("Local video track enabled?", vTrack ? vTrack.enabled : "No track");
        ...
        // Force muted prop on element to be sure
        video.muted = true;
    }

    // FIX: For REMOTE users, we'll wait for video-state message via data channel
    // Don't set video-off by default - let the data channel messages handle this
    if (isLocal) {
...
function showShareLinkModal(meetingId) {
    if (!shareLinkModal || !shareMeetingLink) return;

    const meetingUrl = `${window.location.origin}${window.location.pathname}?join=${meetingId}`;
...
function hideShareLinkModal() {
    if (shareLinkModal) {
        shareLinkModal.classList.remove('active');
    }
}
...
if (copyShareLinkBtn) {
    copyShareLinkBtn.addEventListener('click', () => {
        if (shareMeetingLink) {
            navigator.clipboard.writeText(shareMeetingLink.value).then(() => {
...
// Dismiss button
if (dismissShareLinkBtn) {
    dismissShareLinkBtn.addEventListener('click', () => {
        hideShareLinkModal();
    });
}

// Auto-hide when first user joins - this is called whenever a new participant joins
function onParticipantJoined() {
    // Hide the share link modal when at least one other user joins
    const participantDataCount = Object.keys(participantsData).length;
    if (participantDataCount > 0 && amIHost) {
        hideShareLinkModal();
    }
}
